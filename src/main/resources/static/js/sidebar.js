document.addEventListener('DOMContentLoaded', function() {
  // 부모 엘리먼트에 이벤트 리스너 등록
  // 카테고리 선택하면 icon 클래스가 append() 돼서 이벤트 리스너에 등록이 안돼서 부모 등록함
  document.querySelector('.main__content').addEventListener('click', function(event) {
      // 클릭된 엘리먼트가 icon 클래스를 가지고 있는지 확인
      if (event.target.classList.contains('icon')) {
          // 클릭된 엘리먼트가 icon 클래스를 가지고 있다면 toggleHeartIcon 함수 호출
          ...
          $.ajax({
              ...
              error: function (xhr, status, error) {
                 if (xhr.status === 403) {
                                          ...
                                          location.href = '/members/login/user'; // 로그인 페이지로 이동
                                      } else {
          ...
          '<li class="card">' +
              ...
              '</a>' +
                  '<div class="image-container">' +
                      '<img class="icon" src="' + (item.liked ? '/img/fill_heart.svg' : '/img/empty_heart.svg') + '"data-heart-id="' + item.id + '" data-liked="' + item.liked + '">' +
                  '</div>' +
                  '<div class="card-body">' +
                      '<h4 class="card-title">' + item.itemNm + '</h4>' +
                      '<h3 class="card-price">' + item.price + '원</h3>' +
                  '</div>' +
          '</li>';
